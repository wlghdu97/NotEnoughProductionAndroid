...
) : MediatorUseCase<LoadUsageMachineListUseCase.Parameter, PagedList<RecipeMachineView>>() {

    override fun executeInternal(params: Parameter) = liveData<Resource<PagedList<RecipeMachineView>>> {
        val config = PagedList.Config.Builder()
            .setPageSize(PAGE_SIZE)
            .build()
        val dataSource = elementRepo.getUsageMachinesByElement(params.elementId)
